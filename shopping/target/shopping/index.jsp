...
<script type="text/javascript"
	...
	<script>
		...
		function go(currentPage, PageSize) {
			$.post("${pageContext.request.contextPath}/Product/paging",{
                ...
                $a.eq(1).attr("href", "javascript:go('" + data.paging.prePage + "','" + PageSize + "')");
                $a.eq(2).attr("href", "javascript:go('" + data.paging.nextPage + "','" + PageSize + "')");
                $a.eq(3).attr("href", "javascript:go('" + data.paging.pages + "','" + PageSize + "')");
                $span = $(".btn>span");
                $span.eq(0).html(data.paging.pageNum);
                $span.eq(1).html(data.paging.pages);
            },"json")
        }
	</script>
</head>
